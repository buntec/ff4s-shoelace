...
class ShoelaceGenerator(
  ...
  def cssPropType(cssType: Def.CssType): String = {
    cssType match {
      case WebComponentsDef.CssLengthType =>
        "StyleProp[String] with u.Length[DSP, Int]"
      case WebComponentsDef.CssColorType =>
        "StyleProp[String] with u.Color[SS, DSP]"
  ...
  def printElementFile(
    ...
    enter(
      s"object ${element.scalaName} extends WebComponent {",
      "}"
    ) {

      line(s"override val tagName: String = ${repr(element.tagName)}")
  ...
  def printEventProps(element: Def.Element): Unit = {
    ...
    element.events.foreach { event =>
      ...
      line(
        s"lazy val ${event.scalaName}: EventProp[${eventType}] = EventProp[${eventType}](${repr(event.domName)})"
      )
    }
  }
  ...
  def printProps(element: Def.Element): Unit = {
    ...
    element.writableNonReflectedProperties.foreach { prop =>
      ...
      val propImpl =
        s"${propImplName(scalaInputTypeStr)}(${repr(prop.propName)})"
      blockCommentLines(prop.description)
      line(
        s"lazy val ${prop.propName}: HtmlProp[${scalaInputTypeStr}, _] = ${propImpl}"
      )
    }
  }
