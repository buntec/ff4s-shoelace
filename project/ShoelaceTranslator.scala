...
class ShoelaceTranslator(
  ...
  /** Laminar needs to know which keys are allowed in `controlled` blocks. Web
    * components often have their own name, e.g. Shoelace uses `sl-input`
  ...
  def fixTypes(
    ...
    (tagName, attrNameOrPropName) match {
      case ("sl-alert", "duration") =>
        List(Def.JsNumberType) // Double or Infinity
      case ("sl-animation", "iterations") =>
  ...
  def forceReadonlyField(
    ...
    (tagName, propName, jsTypes) match {
      ...
      case ("sl-option", "current" | "selected" | "hasHover", _) =>
  ...
  private val cssPropTypes = List(
    ...
    CssPropTypePattern("sl-carousel")(
      "^slide-gap|scroll-hint$" -> Def.CssLengthType
    ),
    CssPropTypePattern("sl-split-panel")(
    ...
    CssPropTypePattern("sl-progress-ring")(
  ...
  def elements: List[Def.Element] = {
    manifest.modules.map { module =>
      ...
      if (module.declarations.length != 1) {
        throw new Exception(
          s"Expected exactly one declaration in module `${module.path}`, got ${module.declarations.length}."
        )
      }
  ...
  def attributes(elementDeclaration: M.Declaration): List[Def.Attribute] = {
    elementDeclaration.attributes.flatMap { attr =>
      val jsTypes =
        parseJsTypes(elementDeclaration.tagName, attr.name, attr.`type`)
      if (
        jsTypes.exists(t => isAttributeTypeHtmlCompatible(t).contains(false))
      ) {
  ...
  def CustomEventType(
    ...
    Def.CustomEventType(
      ...
      fields = fields.toList.map { case (domName, jsTypeStr) =>
        ...
        val description =
          if (jsTypes.exists(_.isInstanceOf[Def.JsStringConstantType])) {
            List(jsTypeStr)
          } else Nil
  ...
  def scalaPropInputTypeStr(prop: Def.Field, tagName: String): String = {
    ...
    if (printableTypes.isEmpty) {
      ...
      printableTypes.toSet == Set(
        Def.JsStringType,
        Def.JsCustomType("string[]")
      )
    ) {
  ...
  /** Note: UI library might chose encode reflected attrs as props instead,
    * which might have different names. Use this only for choosing canonical
    * names.
    */
